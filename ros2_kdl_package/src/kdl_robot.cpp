...
void KDLRobot::getInverseKinematics(KDL::Frame &f, KDL::JntArray &q)
{
    ...
    if (ret != 0)
    {
        std::cout << ikSol_->strError(ret) << std::endl;
    };
}
...
void KDLRobot::update(std::vector<double> _jnt_values, std::vector<double> _jnt_vel)
{
    ...
    // joints space
    err = dynParam_->JntToMass(jntArray_, jsim_);
    if (err != 0)
    {
        std::cout << strError(err);
    };
    err = dynParam_->JntToCoriolis(jntArray_, jntVel_, coriol_);
    if (err != 0)
    {
        std::cout << strError(err);
    };
    err = dynParam_->JntToGravity(jntArray_, grav_);
    if (err != 0)
    {
        std::cout << strError(err);
    };
    ...
    // robot flange
    err = fkVelSol_->JntToCart(jntVel, s_Fv_f);
    if (err != 0)
    {
        std::cout << strError(err);
    };
